...
int main()
{
	...
	std::unordered_map<std::string, int>::iterator ptr;
	// this implementation uses unordered_maps
	...
	for (int i = 0; i < (int)list.size(); i++){
   		...
   		ptr = mp.find(list[i]); 
   		if(!(ptr != mp.end()))
   			mp.insert(std::pair<std::string,int>(list[i],1)); // populates unordered_map
		else
   			mp[list[i]] += 1;} // updates unordered_map

   	std::cout << "Freq:\t" << "Word:" << std::endl;
   	for(auto itr = mp.begin(); itr!=mp.end(); itr++)
        std::cout << '[' << itr->second << "]\t" << itr->first << std::endl;
        // words are printed unordered
